...
public:
    int removeDuplicates(vector<int>& nums) {
        ...
        for (int i = 1; i < nums.size(); ++i) {
            ...
            if (nums[i] != nums[k - 1]) {
                // Place it at the next position for unique elements
                nums[k] = nums[i];
                k++; // Increment the unique count
            }
            // If it's the same as the last unique, just skip it
        }

        // k now contains the number of unique elements
        return k;
    }
};
...
Space: O(1) — no extra space used; in-place operations only.

Example:
For input: nums = [0,0,1,1,1,2,2,3,3,4]
